...
import { getFavorites, addFavorite, removeFavorite, isFavorite as checkIsFavorite } from "../services/api";
import { Play, Heart, Share2, ArrowLeft } from "lucide-react";
...
export function VideoDetail({ video, suggestedVideos, onPlay }: VideoDetailProps) {
  ...
  return (
    <div className="video-detail">
      ...
      <div className="video-detail-hero">
        ...
        <div className="video-detail-hero-content">
          ...
          <div className="video-detail-actions">
            <button className="btn btn-play" onClick={() => onPlay(video)}>
              <Play size={20} />
              Play
            </button>
            <button className={`btn btn-circle ${isFav ? "active" : ""}`} onClick={toggleFavorite} aria-label="Add to My List">
          ...
          {video.description && (
            <div className="video-detail-description">
              ...
              {video.description.length > 200 && (
                <button className="read-more-btn" onClick={() => setShowDesc(!showDesc)}>
      ...
      {suggestedVideos.length > 0 && (
        <VideoCarousel title="More Like This" videos={suggestedVideos.slice(0, 15)} onPlay={onPlay} />
      )}
    </div>
  );
}