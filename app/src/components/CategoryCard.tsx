    ...
    <Link to={`/category/${category.key}`} className="category-card">
      <div className="category-card-image">
        {category.squareImageUrl ? (
          <img src={category.squareImageUrl} alt={category.name} loading="lazy" />
        ) : (
      ...
      <div className="category-card-info">
        <h3>{category.name}</h3>
        {category.videoCount !== undefined && <span className="category-count">{category.videoCount} videos</span>}
      </div>
