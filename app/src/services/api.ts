import type { Video, Category, VideoData } from "../types";
...
export async function getVideosByCategory(categoryKey: string): Promise<Video[]> {
  const data = await loadData();
...
export async function getRecentlyPublished(limit = 20): Promise<Video[]> {
  ...
  return data.videos
    ...
    .sort((a, b) => (b.firstPublished ?? "").localeCompare(a.firstPublished ?? ""))
    .slice(0, limit);
}
...
export function getWatchedVideos(): Video[] {
  ...
  return hist.map((k) => data.videos.find((v) => v.key === k)).filter(Boolean) as Video[];
}